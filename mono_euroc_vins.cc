...
int main(int argc, char **argv)
{
    ...
    printf("startimutime: %.11f\n",startimutime);
    printf("imagestarttime: %.11f\n",vTimestamps[0]);
    while(1)
    {
	...
	cout<<vstrImageFilenames[0]<<endl;
    // Main loop
    long imuindex = 0;
    cv::Mat im; //imagestart
   // for(int ni=imagestart; ni<nImages; ni++)
    for(int ni=15000; ni<nImages; ni++)
    {
        if(sigflag) // ctrl-c exit
            break;

        // Read image from file
	cerr << endl << "Reading:"<<  vstrImageFilenames[ni] << endl;
	...
	Mat sharpningKernel8 = (cv::Mat_<float>(3, 3) << -k/9.0f, -k/9.0f,-k/9.0f,-k/9.0f, 1 + (8 * k)/9.0f, -k/9.0f,-k/9.0f, -k/9.0f,-k/9.0f);
	cv::filter2D(im, dst, im.depth(), sharpningKernel8);
	im = dst;

/// contrast1
/*
	Mat image = im;
	image.convertTo(dst,-1,1.8,0);
	im = dst;
*/

/*	cv::cvtColor(im,im,CV_BGR2GRAY);
	Mat hist_image;
	equalizeHist(im,hist_image);
	im = hist_image;
*/
                ...
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ...
    vTimeStamps.reserve(32000);
    vstrImages.reserve(32000);
    //int testcnt=10;
    while(!fTimes.eof())
    {
        string s;
   ...
   //         string tmpstr(strImagePath + "/" + to_string(tmpi) + ".jpg");
		//s.erase(std::remove(s.begin(), s.end(), '\n'), s.end());
	cout<<"s: "<<s<<endl;
	string tmpstr(strImagePath + "/" + s);
	tmpstr.append(".jpg");
// + ".jpg");
	    ...
	    //vTimeStamps.push_back(tmpi);
		vTimeStamps.push_back(std::stod(s)*1e-11);
        }
